...
@app.cell
def __(mo):
    mo.md("""# DECC Weekly Report

Converted from test_file.qmd for presentation.  
R chunks are preserved as rendered code blocks.""")
    return
...
def __(Path):
    data_root = Path("data")
    data_files = sorted(data_root.glob("current_year/*.csv")) + sorted(
        data_root.glob("prior_year/*.csv")
    )

    file_options = {f"{p.parent.name}/{p.name}": str(p) for p in data_files}
    return data_files, file_options


@app.cell
def __(file_options, mo):
    if not file_options:
        mo.md(
            """## Interactive Python Demo

No CSV files were found in data/current_year or data/prior_year."""
        )
        file_selector = None
    else:
        default_value = next(iter(file_options.values()))
        file_selector = mo.ui.dropdown(
            options=file_options,
            value=default_value,
            label="Select weekly CSV",
            full_width=True,
        )
        mo.vstack(
            [
                mo.md("## Interactive Python Demo"),
                mo.md(
                    "Use this section for a live walkthrough during presentation."
                ),
                file_selector,
            ]
        )
...
@app.cell
def __(file_selector, pd):
    if file_selector is None:
        selected_path = None
        demo_df = pd.DataFrame()
    else:
        selected_path = file_selector.value
        demo_df = pd.read_csv(selected_path, dtype=str, low_memory=False)

    return demo_df, selected_path


@app.cell
def __(demo_df, mo, pd, selected_path):
    ...
    if demo_df.empty:
        ...
        priority_col = pick_column(demo_df, ["priority", "pri", "priority_level"])
        agency_col = pick_column(demo_df, ["agency", "discipline", "service"])

        top_priority = (
            demo_df[priority_col].fillna("(missing)").value_counts().head(1).index[0]
            if priority_col
            else "N/A"
        )
        top_agency = (
            demo_df[agency_col].fillna("(missing)").value_counts().head(1).index[0]
            if agency_col
            else "N/A"
        )

        mo.md(
            f"""### Quick KPI Snapshot

- File: {selected_path}
- Calls (rows): {len(demo_df):,}
- Fields (columns): {len(demo_df.columns):,}
- Most frequent priority: {top_priority}
- Most frequent agency/discipline: {top_agency}
"""
        )
...
def __(demo_df, mo, pd, plt):
    def detect_datetime_column(df: pd.DataFrame) -> str | None:
        ...
        candidates = [
            c for c in df.columns if any(tag in c.lower() for tag in datetime_like_names)
        ]
        candidates += [c for c in df.columns if c not in candidates]

        for col in candidates[:30]:
    ...
    dt_col = detect_datetime_column(demo_df)
    if dt_col is None:
        mo.md(
            "Could not detect a datetime field for hourly trend plotting in this dataset."
        )
        return

    parsed = pd.to_datetime(demo_df[dt_col], errors="coerce")
    hours = parsed.dt.hour.dropna().astype(int)

    if hours.empty:
        mo.md("Detected datetime field has no parseable values.")
        return
    ...
    ax.plot(hour_counts["hour"], hour_counts["calls"], marker="o", linewidth=2)
    ax.set_title(f"Hourly Call Volume (derived from: {dt_col})")
...
- **[Executive Summary](#executive-summary)**
  - [Weekly KPI Summary](#weekly-kpi-summary)
...
- **[Exploratory Analysis](#exploratory-analysis)**
  ...
  - [CAD-centric Analyses](#cad-centric-analyses)
    ...
    - [Agency/Discipline Distribution](#agency-distribution)
    - [Call Reception Distribution](#call-reception-distribution)
    - [Disposition Codes Analysis](#disposition-codes)
    - [Problem Type Distribution](#problem-type-top10)
    - [Call Taker Performance](#call-taker-top10)
    - [Time on Phone Analysis](#time-on-phone)
  - [Multi-Week Performance Trends](#multi-week-performance-trends)
    - [Trend Charts](#trend-charts)
    - [Dual-Axis Volume & Efficiency](#dual-axis-trends)
  - [Priority-Level Efficiency Analysis](#priority-level-efficiency-analysis)
  - [Call Type Complexity Analysis](#call-type-complexity-analysis)

### Strategic Analytics

- **[Advanced Analytics & Strategic Insights](#advanced-analytics--strategic-insights)**
  - [All Dispatcher Performance Analysis](#all-dispatcher-performance-analysis)
    - [Automated/CAD-to-CAD/Null Dispatches](#automated-cad-null-summary)
    - [Workload Distribution](#workload-distribution)
  - [Staff Development Insights](#staff-development-insights)
    - [Top Performers](#top-performers-fastest-median-ttd)
    - [Development Opportunities](#development-opportunities-slowest-median-ttd)
  - [Automated Recommendations](#automated-recommendations)
    - [Priority Action Items](#recommendations-output)
  - [Process Improvement Opportunities](#process-improvement-opportunities)

### Detailed Time Metrics

- **[Summary Statistics and Analyses](#summary-statsitcs-and-analyses)**
  - [Elapsed Time Summary (Tabbed)](#elapsed-time-summary-tabset)
    - [Overall](#overall)
    - [By Agency](#by-agency)
...
- **[Discipline Analyses](#discipline-analyses)**
  - [APD Analyses](#apd-analyses)
    - [Day of Week Distribution](#apd-dow-distribution)
    - [Hour of Day Distribution](#apd-hour-distribution)
    - [Call Reception Breakdown](#apd-reception-distribution)
    - [Problem Type Distribution](#apd-problem-distribution)
  - [AFD FIRE Analyses](#afd-fire-analyses)
    - [Day of Week Distribution](#afd-fire-dow-distribution)
    - [Hour of Day Distribution](#afd-fire-hour-distribution)
    - [Call Reception Breakdown](#afd-fire-reception-distribution)
    - [Problem Type Distribution](#afd-fire-problem-distribution)
  - [AFD EMS Analyses](#afd-ems-analyses)
    - [Day of Week Distribution](#afd-ems-dow-distribution)
    - [Hour of Day Distribution](#afd-ems-hour-distribution)
    - [Call Reception Breakdown](#afd-ems-reception-distribution)
    - [Problem Type Distribution](#afd-ems-problem-distribution)

### Specialized Call Analyses
...
- **[E-911 Service Call Analyses](#e-911-service-call-analyses)**
  ...
  - [Non 9-1-1 Call Breakdowns](#non-9-1-1-call-breakdowns)

- **[Topical Analyses](#topical-analyses)**
...
- **[Outlier Identification](#outlier-identification)**
  - [Critical Priority Outlier Alerts](#critical-priority-outlier-alerts)
  - [General Outlier Analysis](#general-outlier-analysis)
  - [Detailed Outliers Information](#detailed-outliers-information)
  - [Outlier Impact](#outlier-impact)
  - [Comparison: Overall vs. Outlier Statistics](#comparison-overall-vs-outlier-statistics)

### Advanced Investigations

- **[Correlation Studies](#correlation-studies)**

- **[Time to Queue and Dispatch Scatter Analysis](#time-to-queue-and-dispatch-scatter-analysis)**
  - [Overall TTQ vs TTD Relationship](#overall-ttq-vs-ttd-relationship)
  - [TTQ vs TTD by Discipline](#ttq-vs-ttd-by-discipline)
  - [TTQ vs TTD by Priority Level](#ttq-vs-ttd-by-priority-level)
  - [TTQ vs TTD for 9-1-1 Calls](#ttq-vs-ttd-for-9-1-1-calls)
...
- **[Comparisons](#comparisons)**
  ...
  - [Outlier Comparisons](#outlier-comparisons)

### Forecasting

- **[Week Ahead Forecast](#week-ahead-forecast)**
...
# ============================================================================
# USER CONFIGURATION - WEEK SELECTION
# ============================================================================
# Choose how to determine which week to analyze:
# 
# OPTION 1: Automatic (default) - Uses current date to calculate week
# Set USE_MANUAL_WEEK <- FALSE
#
# OPTION 2: Manual - Specify exact week number and year to analyze
# Set USE_MANUAL_WEEK <- TRUE and set MANUAL_WEEK_NUM and MANUAL_WEEK_YEAR

USE_MANUAL_WEEK <- FALSE        # Set to TRUE to manually specify week
MANUAL_WEEK_NUM <- 6            # Week number to analyze (1-52)
MANUAL_WEEK_YEAR <- 2026        # Year of the week to analyze
...
if (USE_MANUAL_WEEK) {
  # Manual mode: Use user-specified week
  cat("=== MANUAL WEEK SELECTION MODE ===\n")
  cat("Using manually specified week:", MANUAL_WEEK_NUM, "of year", MANUAL_WEEK_YEAR, "\n\n")
  
  # Set current week values directly from manual input
  current_week_num <- MANUAL_WEEK_NUM
  current_year <- MANUAL_WEEK_YEAR
  
  # Calculate the actual start/end dates for display purposes
  # ISO week 1 is the week containing the first Thursday of the year
  jan1 <- as.Date(paste0(MANUAL_WEEK_YEAR, "-01-01"))
  jan1_dow <- wday(jan1, week_start=7)  # 1=Sunday, 7=Saturday
  
  # Find the first Sunday of ISO week 1
  days_to_first_sunday <- (7 - jan1_dow + 1) %% 7
  if (wday(jan1 + days(4 - days_to_first_sunday), week_start=7) == 5) {  # Check if Thursday is in first week
    first_sunday <- jan1 + days(days_to_first_sunday)
  } else {
    first_sunday <- jan1 + days(days_to_first_sunday + 7)
  }
  
  current_week_start <- first_sunday + weeks(MANUAL_WEEK_NUM - 1)
  current_week_end <- current_week_start + days(6)
  
  # Calculate comparison week (one week before) - simple arithmetic
  if (MANUAL_WEEK_NUM > 1) {
    last_week_num <- MANUAL_WEEK_NUM - 1
    last_year <- MANUAL_WEEK_YEAR
  } else {
    # Week 1 wraps to previous year's last week (usually 52, sometimes 53)
    last_year <- MANUAL_WEEK_YEAR - 1
    # Check if previous year had 53 weeks
    dec31_prev <- as.Date(paste0(last_year, "-12-31"))
    last_week_num <- isoweek(dec31_prev)
  }
  ...
  REPORT_DATE <- current_week_end + days(1)  # Set report date to day after week ends
  
  # Set report week variables for compatibility with other code
  report_week_start <- current_week_end + days(1)  # Report week starts day after current week ends
  report_week_num <- isoweek(report_week_start + days(3))
  report_year <- isoyear(report_week_start + days(3))
  
} else {
  # Automatic mode: Calculate from today's date
  ...
  current_year <- isoyear(current_week_start + days(3))
  ...
  last_year <- isoyear(last_week_start + days(3))
}
...
load_week_data <- function(week_num, week_year) {
  # Determine which folder to look in based on the week's year
  if (week_year >= year(REPORT_DATE)) {
    # Current or future year - look in data/current_year
  ...
  week_num_padded <- sprintf("%02d", week_num)
  possible_files <- c(
    file.path(base_path, paste0("week", week_num_padded, ".csv")),
...
load_phone_data <- function(week_num, week_year) {
  ...
  # Try different naming conventions for phone data (including zero-padded)
  week_num_padded <- sprintf("%02d", week_num)
  possible_files <- c(
    file.path(base_path, paste0("week_", week_num_padded, "_phone.csv")),
...
for (i in 1:4) {
  ...
  week_offset_year <- isoyear(week_offset_start + days(3))
...
def __(mo):
    mo.md("""```r
#| label: display-week-info
#| echo: false
#| output: asis

cat("\n## Week", WEEK_NUMBER, "from", WEEK_START_FORMATTED, "through", WEEK_END_FORMATTED, current_year, "\n\n")
```""")
    return
...
knitr::opts_chunk$set(
  cache = FALSE,          # disable caching to avoid path issues
...
This is the weekly report for week `r WEEK_NUMBER` covering the period from `r WEEK_START_FORMATTED` 2025 through `r WEEK_END_FORMATTED` 2026. The report will include analyses of the data to emphasize different information that is contained within the data and may be pertinent to both operations and management.""")
    return
...
df <- df |>
  mutate(across(c(Time_To_Queue, Time_To_Dispatch, Phone_Time, Processing_Time, 
                  Rollout_Time, Transit_Time, Total_Call_Time), as.numeric))
...
@app.cell
def __(mo):
    mo.md("""For this week, there were a total of `r nrow(df)` calls for service. 
...
@app.cell
def __(mo):
    mo.md("""**Status Legend:** ✓ = Meets Target | ⚠ = Warning | ✗ = Below Target | — = Not Applicable
...
if (!is.null(current_week_phone)) {
  ...
  total_phone_calls <- total_911_calls + total_admin_calls
  ...
  total_abandoned <- total_911_abandoned + total_adm_abandoned
  
  # Calculate answered calls
  answered_911_calls <- total_911_calls - total_911_abandoned
  answered_admin_calls <- total_admin_calls - total_adm_abandoned
  total_answered <- total_phone_calls - total_abandoned
  
  # Calculate 911 answer time percentages from actual call counts (not mean of percentages)
  within_911_10 <- sum(current_week_phone$`911_10`, na.rm = TRUE)
  within_911_15 <- within_911_10 + sum(current_week_phone$`911_15`, na.rm = TRUE)
  within_911_20 <- within_911_15 + sum(current_week_phone$`911_20`, na.rm = TRUE)
  
  avg_911_pct_10 <- if (answered_911_calls > 0) (within_911_10 / answered_911_calls) * 100 else 0
  avg_911_pct_15 <- if (answered_911_calls > 0) (within_911_15 / answered_911_calls) * 100 else 0
  avg_911_pct_20 <- if (answered_911_calls > 0) (within_911_20 / answered_911_calls) * 100 else 0
  
  # Calculate admin answer time percentages from actual call counts
  within_adm_10 <- sum(current_week_phone$ADM_10, na.rm = TRUE)
  within_adm_15 <- within_adm_10 + sum(current_week_phone$ADM_15, na.rm = TRUE)
  within_adm_20 <- within_adm_15 + sum(current_week_phone$ADM_20, na.rm = TRUE)
  
  avg_adm_pct_10 <- if (answered_admin_calls > 0) (within_adm_10 / answered_admin_calls) * 100 else 0
  avg_adm_pct_15 <- if (answered_admin_calls > 0) (within_adm_15 / answered_admin_calls) * 100 else 0
  avg_adm_pct_20 <- if (answered_admin_calls > 0) (within_adm_20 / answered_admin_calls) * 100 else 0
  
  # Calculate overall answer time percentages from actual call counts
  within_total_10 <- within_911_10 + within_adm_10
  within_total_15 <- within_911_15 + within_adm_15
  within_total_20 <- within_911_20 + within_adm_20
  
  avg_total_pct_10 <- if (total_answered > 0) (within_total_10 / total_answered) * 100 else 0
  avg_total_pct_15 <- if (total_answered > 0) (within_total_15 / total_answered) * 100 else 0
  avg_total_pct_20 <- if (total_answered > 0) (within_total_20 / total_answered) * 100 else 0
  
  # Calculate abandoned call percentages
...
if (phone_data_available) {
  ...
  if (exists("total_abandoned") && total_abandoned > 0) {
    cat(paste0("- **Total Abandoned (All Lines):** ", format(total_abandoned, big.mark = ","), 
               " (", round((total_abandoned/total_phone_calls)*100, 2), "% of all calls)\n\n"))
  } else {
    cat("\n")
  }
...
build_phone_weekly_summary <- function(phone_df, week_num, week_start) {
  ...
  # Use 911_T and 911_AB columns (matching compute_phone_metrics for consistency)
  total_calls <- sum(phone_df$`911_T`, na.rm = TRUE)
  abandoned_911 <- sum(phone_df$`911_AB`, na.rm = TRUE)
  answered_calls <- total_calls - abandoned_911
  
  if (answered_calls <= 0) return(NULL)
  
  # Calculate answer time percentages for 911 calls
  within_10 <- sum(phone_df$`911_10`, na.rm = TRUE)
  within_15 <- within_10 + sum(phone_df$`911_15`, na.rm = TRUE)
  within_20 <- within_15 + sum(phone_df$`911_20`, na.rm = TRUE)
  
  # Calculate abandoned percentage (911 only, for consistency)
  abandoned_pct <- (abandoned_911 / total_calls) * 100
  ...
  tibble(
    ...
    abandoned_total = abandoned_911,
    abandoned_pct = abandoned_pct,
    within_10 = within_10,
    within_15 = within_15,
    within_20 = within_20,
    pct_10 = (within_10 / answered_calls) * 100,
    pct_15 = (within_15 / answered_calls) * 100,
    pct_20 = (within_20 / answered_calls) * 100
  )
}
...
if (!is.null(phone_trends_df) && nrow(phone_trends_df) > 0) {
  ...
  volume_data <- phone_trends_df %>%
    mutate(week_label = paste0("Week ", week_num)) %>%
    dplyr::select(week_label, week_start, total_calls, answered_calls) %>%
    pivot_longer(cols = c(total_calls, answered_calls), names_to = "metric", values_to = "count") %>%
    mutate(metric_label = case_when(
      metric == "total_calls" ~ "Total Calls",
      metric == "answered_calls" ~ "Answered Calls"
    ))
  ...
  p_volume <- ggplot(volume_data, aes(x = reorder(week_label, week_start), y = count, fill = metric_label)) +
    ...
    geom_text(aes(label = scales::comma(count)), 
              ...
              vjust = -0.3, size = 3) +
    scale_fill_manual(values = c("Total Calls" = palette_cyan_base[8], 
                                   "Answered Calls" = palette_blue_base[7])) +
    labs(title = "Call Volumes Over Last 4 Weeks",
         x = "Week",
         y = "Call Count",
         fill = "Metric") +
    theme_minimal() +
...
if (!is.null(phone_trends_df) && nrow(phone_trends_df) > 0) {
  # Ensure proper ordering by week_start and verify data
  phone_trends_plot <- phone_trends_df %>% 
    arrange(week_start) %>%
    mutate(week_label_ordered = factor(week_label, levels = unique(week_label)))
  
  # Get max value for consistent y-axis scaling
  max_abandoned <- max(phone_trends_plot$abandoned_total, na.rm = TRUE)
  
  p_abandoned_count <- ggplot(phone_trends_plot, aes(x = week_label_ordered, y = abandoned_total)) +
    geom_col(fill = "#8B0000", width = 0.7, alpha = 0.85) +
    geom_text(aes(label = scales::comma(abandoned_total)),
              vjust = -0.5, size = 3.5, fontface = "bold", color = "#8B0000") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.1)), 
                       limits = c(0, max_abandoned * 1.1)) +
    labs(title = "Total Abandoned Calls (9-1-1 + Admin) – Counts",
         x = "Week",
...
@app.cell
def __(mo):
    mo.md("""### Call Distribution: Hour by Day of Week
...
@app.cell
def __(mo):
    mo.md("""The heat map shows one intense spike on Tuesday at 1000 hours. There is an additional spike on Wednesday at 1000 hours. Overall, since `r busiest_hour`00 hours was the busiest hour of the week, the heatmap shows that it was reasonably consistent throughout the week. 
...
@app.cell
def __(mo):
    mo.md("""As expected, day shifts are busier than night shifts by a considerable margin. As expected, the call volumes throughout the shift for day shift are fairly consistent and on night shift, the longer the shifts go, fewer calls are received. These are both expected from prior work.""")
    return
...
@app.cell
def __(mo):
    mo.md("""As expected, the majority of calls are for `r busiest_agency`. They represent `r police_percentage`
...
@app.cell
def __(mo):
    mo.md("""As can be seen from the bar chart, `r mostused_dispo` was the most common disposition code used with `r mostused_dispo_count` calls coded that way. It is interesting to note that no calls, this week, have been closed without a disposition.""")
    return
...
@app.cell
def __(mo):
    mo.md("""The following is a chart of the top 10 call types. The data is limited to ensure visual clarity and legibility of the information.""")
    return
...
@app.cell
def __(mo):
    mo.md("""This week, the most common problem nature was `r busiest_ct`. For AFD, the most common was `r agency_top_problem_names["FIRE"]`. The most common medical call for service was `r agency_top_problem_names["EMS"]`.
...
@app.cell
def __(mo):
    mo.md("""It is interesting to note that the top "call taker" is `r busiest_tc` again this week with `r busiest_tc_count` calls.""")
    return
...
@app.cell
def __(mo):
    mo.md("""This table shows the call takers with the highest cumulative time on the phone. This week, `r longest_phone_ct` had the largest average time on the phone with an average of `r longest_phone_ct_mean` seconds per call. This could be due to a number of factors, including the complexity of the calls faced.
...
for (i in 0:(weeks_to_analyze - 1)) {
  ...
  week_offset_start <- current_week_start - weeks(i)
  week_to_load <- isoweek(week_offset_start + days(3))
  year_to_load <- isoyear(week_offset_start + days(3))
  
  # Try to load using the load_week_data function
  tryCatch({
    week_df <- load_week_data(week_to_load, year_to_load)
    ...
    cat("Warning: Could not load week", week_to_load, "of", year_to_load, "\n")
  })
}
...
if (length(historical_data) > 0) {
  ...
  cat("Warning: No historical data available for trend analysis\n")
  df_historical <- NULL
  weekly_metrics <- NULL
}
```""")
...
if (!is.null(weekly_metrics) && nrow(weekly_metrics) > 0) {
  ...
  cat("*Insufficient historical data for trend visualization.*\n")
}
```""")
    return
...
@app.cell
def __(mo):
    mo.md("""The trend charts above show performance metrics over the last 4 weeks, allowing identification of patterns and emerging issues. Week `r current_week_num` shows `r ifelse(call_volume_change > 0, "an increase", "a decrease")` of `r abs(call_volume_change)` calls compared to week `r last_week_num`.
...
if (!is.null(weekly_metrics) && nrow(weekly_metrics) > 0) {
  ...
  cat("*Insufficient historical data for dual-axis trend visualization.*\n")
}
```""")
    return
...
@app.cell
def __(mo):
    mo.md("""This dual-axis chart allows quick visual assessment of whether call volume changes correlate with dispatch efficiency. Inverse relationships (volume ↑, TTD ↑) may indicate capacity constraints.
...
@app.cell
def __(mo):
    mo.md("""### Call Type Complexity Analysis""")
    return
...
@app.cell
def __(mo):
    mo.md("""## Advanced Analytics & Strategic Insights
...
@app.cell
def __(mo):
    mo.md("""#### Automated/CAD-to-CAD/Null Dispatches (Separate View)
...
@app.cell
def __(mo):
    mo.md("""### Staff Development Insights""")
    return
...
@app.cell
def __(mo):
    mo.md("""#### Top Performers (Fastest Median TTD)""")
    return
...
@app.cell
def __(mo):
    mo.md("""#### Development Opportunities (Slowest Median TTD)""")
    return
...
@app.cell
def __(mo):
    mo.md("""**Coaching Recommendations:**
...
@app.cell
def __(mo):
    mo.md("""### Process Improvement Opportunities""")
    return
...
# Build long form with Priority_Number (ensure numeric ordering)
long_pri <- bind_rows(
  df |> transmute(metric = "Time to Queue",     value = suppressWarnings(as.numeric(Time_To_Queue)),    Priority = suppressWarnings(as.numeric(Priority_Number))),
  df |> transmute(metric = "Time to Dispatch",  value = suppressWarnings(as.numeric(Time_To_Dispatch)), Priority = suppressWarnings(as.numeric(Priority_Number))),
  df |> transmute(metric = "Phone Time",        value = suppressWarnings(as.numeric(Phone_Time)),       Priority = suppressWarnings(as.numeric(Priority_Number))),
  df |> transmute(metric = "Processing Time",   value = suppressWarnings(as.numeric(Processing_Time)),  Priority = suppressWarnings(as.numeric(Priority_Number)))
...
stats_pr <- long_pri |>
  ...
  arrange(metric, Priority) |>
  mutate(Priority = as.character(Priority))

ann_pr <- stats_pr |>
  transmute(metric, Priority = factor(Priority, levels = sort(unique(na.omit(as.numeric(stats_pr$Priority))))), 
            has_sla = metric %in% c("Time to Queue","Time to Dispatch"),
            med = Median, p90 = P90)

# Convert Priority to factor for proper ordering in plot
long_pri_plot <- long_pri |>
  mutate(Priority = factor(Priority, levels = sort(unique(na.omit(long_pri$Priority)))))

ecdf_priority <- ggplot(long_pri_plot, aes(x = value, color = Priority)) +
  stat_ecdf(linewidth = 0.8) +
  facet_wrap(~ metric, scales = "free_x") +
...
@app.cell
def __(mo):
    mo.md("""By defining these datasets, we can now add to our analyses. For example, we can reuse the same information from above to drill down into APD and AFD calls. Starting with APD calls for service, we can examine everything as we did above.
...
@app.cell
def __(mo):
    mo.md("""This week, `r busiest_day_law` was the busiest day of the week for APD service calls. `r slowest_day_law`, being the lightest day of the week overall, was the lightest day for the APD as well.""")
    return
...
@app.cell
def __(mo):
    mo.md("""The busiest hours for the week were 1000 and 1700 hours. There is still a consistent pattern to the morning and afternoon rush hours being the busiest times of the week.""")
    return
...
@app.cell
def __(mo):
    mo.md("""As can be seen, the majority of calls came through `r busiest_cr_law`. This stands in contrast to `r busiest_cr` being the vehicle for the most calls overall for the week.""")
    return
...
@app.cell
def __(mo):
    mo.md("""The largest call type was for `r busiest_prob_law`, which was also the largest call type for the week overall. This could be something to monitor over time to see how the trend changes over time.""")
    return
...
@app.cell
def __(mo):
    mo.md("""As expected, the largest number of calls were Priority `r busiest_pri_law` calls which represent `r busiest_pri_law_percentage` percent of all APD calls. Again, this comports with the overall weekly trends.""")
    return
...
@app.cell
def __(mo):
    mo.md("""This week, the busiest day for fire-related calls was `r busiest_day_fire` with `r busiest_day_count_fire` calls for service. `r slowest_day_fire` was the lightest day for fire-related calls with `r slowest_day_count_fire` calls for service.""")
    return
...
@app.cell
def __(mo):
    mo.md("""The greatest number of fire-related service calls were for `r busiest_prob_fire`. That is an interesting observation and should be watched through the future.""")
    return
...
@app.cell
def __(mo):
    mo.md("""The most-used priority for fire-related calls was P`r busiest_pri_fire`. P1 calls account for `r p1_fire_percentage` percent of all fire-related calls this week.""")
    return
...
@app.cell
def __(mo):
    mo.md("""Overall, DECC operations appear to be very efficient at getting fire-related service calls out to the field. The median processing time was only `r median_processing_time` seconds. This shows that we can easily be in compliance with all necessary NENA and NFPA guidelines. The median time on the phone was `r median_phone_time` seconds. The mean time was `r mean_phone_time` seconds, which is still amazing.
...
@app.cell
def __(mo):
    mo.md(""" This week, there is a spike in medical calls on `r busiest_day_ems`. This appears to correlate to the information that we saw earlier in the report. Outside of `r busiest_day_ems`, the remainder of the week appears to be consistent for the number of medical calls handled.""")
    return
...
@app.cell
def __(mo):
    mo.md("""This week, the busiest hour was `r busiest_hour_ems` hours. The afternoon to evening hours, this week, stayed consistently busy which appears to continue the trends previously observed.""")
    return
...
@app.cell
def __(mo):
    mo.md("""As expected, the vast majority of medical calls arrived via `r busiest_cr_ems`. However, `r cr_nr_pct_ems` percent of medical calls arrived without a method by which we recevied the call. We should continue to monitor and investigate why these are occurring.""")
    return
...
@app.cell
def __(mo):
    mo.md("""The most prevalent medical service type this week was `r busiest_prob_med`. Further we had `r ems_ma_call` mutual aid medical calls this week.""")
    return
...
@app.cell
def __(mo):
    mo.md(""" The majority of medical service calls are P`r busiest_pri_ems`, which is to be expected.""")
    return
...
@app.cell
def __(mo):
    mo.md("""This list is fairly short and could simply be monitored in future should the need arise.
...
@app.cell
def __(mo):
    mo.md("""Almost all of the problem types in this graph belong to AFD and are medical calls. Based on the information above, this is to be expected.
...
@app.cell
def __(mo):
    mo.md("""## E-911 Service Call Analyses
...
@app.cell
def __(mo):
    mo.md("""The table above shows the information about the handling of calls that arrived by 9-1-1 trunk lines. The median time to process those calls for dispatch was `r median_processing_time_911` seconds.""")
    return
...
@app.cell
def __(mo):
    mo.md("""As can be seen `r busiest_day_911` was the busiest day for E-911 calls this week. This is inline with the busiest day of the week for all calls to the center.""")
    return
...
@app.cell
def __(mo):
    mo.md("""This shows that while the overall pattern for 9-1-1 calls is in keeping with the general weekly trend. However, the spike at `r busiest_hour_911`00 hours stands out. Was there a significant call for service during that hour that generated a lot of calls on the 9-1-1 lines for the floor?
...
@app.cell
def __(mo):
    mo.md("""As can be seen `r busiest_day_non911` was the busiest day for Non-E-911 calls this week. This is inline with the busiest day of the week for all calls to the center.""")
    return
...
@app.cell
def __(mo):
    mo.md("""As we can see, with a very limited number of cardiac arrest calls for the week.""")
    return
...
@app.cell
def __(mo):
    mo.md("""The busiest day of the week for mental health calls was `r busiest_day_mh` with `r busiest_day_count_mh` service calls.""")
    return
...
@app.cell
def __(mo):
    mo.md("""Most of these calls arrived, for this past week, in the late mornings through evenings. Again, should this data prove to be part of a trend, then we should adjust the availability of repsonders to address the community's needs.""")
    return
...
@app.cell
def __(mo):
    mo.md("""This week, most of our mental health calls `r busiest_cr_mh` Further analysis could be understaken to determine if any of these are transfer calls from our local 988 provider partner.""")
    return
...
@app.cell
def __(mo):
    mo.md("""The most used call type was `r busiest_ct_mh` which is expected.""")
    return
...
@app.cell
def __(mo):
    mo.md("""Since `r busiest_ct_mh` was the most used call type and is a P2 call, Priority `r busiest_pri_mh` is the most used priority. The question, in the future, will be does these calls need to changed to a higher priority?""")
    return
...
@app.cell
def __(mo):
    mo.md("""The majority of calls with no source were for `r nrr_busiest_agency` with `r nrr_busiest_agency_count` calls for service. This week, that is substatially higher than the other two agencies combined.""")
    return
...
@app.cell
def __(mo):
    mo.md("""From this we can see that `r busiest_hct` took the most calls among the hybrid call takers with `r busiest_hct_count` calls for service. 
...
@app.cell
def __(mo):
    mo.md("""This chart shows that `r most_used_hct_recd` was the method by which the Hybrid Call Takers received the most calls with `r most_used_hct_recd_count` calls for service. This represents `r round(most_used_hct_recd_count / nrow(df_hybrid) * 100, 2)`% of calls received by the hybrid call takers. 
...
@app.cell
def __(mo):
    mo.md("""### General Outlier Analysis""")
    return
...
@app.cell
def __(mo):
    mo.md("""The graph shows that `r ttq_out_busiest_tc` had the most outlier calls with `r ttq_out_busiest_tc_count` calls. Overall there were a small number of call takers that had outlier values in getting a call sent for dispatch.""")
    return
...
@app.cell
def __(mo):
    mo.md("""As expected, all of the calls that have an outlier in the time to dispatch the calls belonged to `r ttd_out_busiest_agency`. This was expected because AFD FIRE and EMS calls use auto-dispatch and rarely wait for more than a few seconds.
...
@app.cell
def __(mo):
    mo.md("""### Comparison: Overall vs. Outlier Statistics
...
@app.cell
def __(mo):
    mo.md("""It is interesting to see that removing the outliers had the opposite effect and seems to show that they do not have a significant impact on our summary statistics. This is likely because we have only removed `r nrow(outlier_df)` rows from the dataset when recalculating
...
@app.cell
def __(mo):
    mo.md("""Key takeaways from these results:
...
@app.cell
def __(mo):
    mo.md("""## Time to Queue and Dispatch Scatter Analysis
...
@app.cell
def __(mo):
    mo.md("""**Discipline-Specific Insights:**""")
    return
...
@app.cell
def __(mo):
    mo.md("""### TTQ vs TTD by Priority Level""")
    return
...
p_priority <- ggplot(priority_data, aes(x = TTQ_capped, y = TTD_capped, color = Priority_Number)) +
  ...
  scale_color_manual(values = c(
    "1" = "#1b9e77",  # teal/green
    "2" = "#7570b3",  # purple
    "3" = "#d95f02",  # orange (darker)
    "4" = "#e7298a"   # magenta for strong contrast on white
  )) +
  labs(
...
@app.cell
def __(mo):
    mo.md("""### TTQ vs TTD for 9-1-1 Calls""")
    return
...
@app.cell
def __(mo):
    mo.md("""## Week Ahead Forecast""")
    return

@app.cell
def __(mo):
    mo.md("""```r
#| label: week-ahead-forecast
#| echo: false
#| message: false
#| warning: false
#| results: asis

# Ensure packages for this section
needed_pkgs <- c("dplyr","tidyr","lubridate","stringr","readr")
...
suppressPackageStartupMessages({
  library(dplyr); library(tidyr); library(lubridate); library(stringr); library(readr)
})

# Define forecast window: next Sunday..Saturday relative to REPORT_DATE
future_start <- report_week_start
future_end <- future_start + days(6)

# Helper: estimate number of distinct ISO weeks covered
n_weeks <- function(df_dates) {
  if (is.null(df_dates)) return(0L)
  n_distinct(isoweek(as.Date(df_dates)))
}

# ============================================================================
# CAD EVENT FORECASTS - TOTAL EVENTS (all agencies combined)
# ============================================================================
cad_source <- last_4_weeks_combined
if (is.null(cad_source)) cad_source <- current_week

cad_daily_forecast <- NULL
if (!is.null(cad_source)) {
  # Use same approach as dow-distribution chunk - just count rows by DOW
  # Assumes DOW and Hour columns already exist and are properly formatted
  
  # Check if DOW column exists, if not create it
  if (!"DOW" %in% names(cad_source)) {
    cad_source <- cad_source |>
      mutate(
        TS_temp = if (inherits(Incident_Start_Time, "POSIXct")) {
          Incident_Start_Time
        } else {
          suppressWarnings(lubridate::mdy_hms(Incident_Start_Time))
        },
        DOW = factor(toupper(format(TS_temp, "%a")), levels = c("SUN","MON","TUE","WED","THU","FRI","SAT"), ordered = TRUE)
      )
  }
  
  # Filter to valid agencies
  cad_ts <- cad_source |>
    filter(Agency %in% c("POLICE","FIRE","EMS"), !is.na(DOW))
  
  # Calculate daily counts using same method as dow-distribution chunk
  dow_counts <- cad_ts |>
    count(DOW, sort = FALSE)
  
  # Get number of unique dates per DOW to calculate average
  # Try Response_Date first, then Incident_Start_Time
  date_col <- NULL
  if ("Response_Date" %in% names(cad_ts)) {
    date_col <- "Response_Date"
  } else if ("Incident_Start_Time" %in% names(cad_ts)) {
    date_col <- "Incident_Start_Time"
  }
  ...
  if (!is.null(date_col)) {
    ...
    dow_daily_avg <- dow_counts |>
      left_join(date_counts, by = "DOW") |>
      mutate(
        TotalEvents = round(n / pmax(1, n_dates), 0)
      ) |>
      dplyr::select(DOW, TotalEvents)
  } else {
    # Fallback: assume ~5 occurrences of each DOW
    dow_daily_avg <- dow_counts |>
      mutate(TotalEvents = round(n / 5, 0)) |>
      dplyr::select(DOW, TotalEvents)
  }
  
  cad_daily_forecast <- dow_daily_avg
}

# PHONE BASELINES (9-1-1 and Admin)
phone_source <- last_4_weeks_phone_combined
if (is.null(phone_source)) phone_source <- current_week_phone
phone_baseline <- NULL
if (!is.null(phone_source)) {
  phone_tmp <- phone_source |>
    mutate(DateTime = suppressWarnings(lubridate::mdy_hm(DateTime))) |>
    mutate(DOW = factor(toupper(format(DateTime, "%a")), levels = c("SUN","MON","TUE","WED","THU","FRI","SAT"), ordered = TRUE),
           Hour = sprintf("%02d", lubridate::hour(DateTime))) |>
    mutate(across(-c(DateTime, DOW, Hour), ~ suppressWarnings(as.numeric(.))))
  weeks_count <- max(1L, n_weeks(phone_tmp$DateTime))
  phone_baseline <- phone_tmp |>
    group_by(DOW, Hour) |>
    summarise(Calls_911 = sum(`911_T`, na.rm = TRUE)/weeks_count,
              Calls_Admin = sum(ADM_T, na.rm = TRUE)/weeks_count,
              .groups = "drop") |>
    mutate(across(c(Calls_911, Calls_Admin), ~ round(., 0)))
}

# Construct daily forecasts (7 rows) - combine phone and CAD
week_ahead_daily <- tibble(Date = seq(future_start, future_end, by = "1 day")) |>
  mutate(DOW = factor(toupper(format(Date, "%a")), levels = c("SUN","MON","TUE","WED","THU","FRI","SAT"), ordered = TRUE))

# Add phone baseline (summed by DOW)
if (!is.null(phone_baseline)) {
  phone_daily <- phone_baseline |>
    group_by(DOW) |>
    summarise(Calls_911 = sum(Calls_911, na.rm=TRUE),
              Calls_Admin = sum(Calls_Admin, na.rm=TRUE), .groups="drop")
  week_ahead_daily <- week_ahead_daily |>
    left_join(phone_daily, by = "DOW")
}

# Add CAD forecast
if (!is.null(cad_daily_forecast)) {
  week_ahead_daily <- week_ahead_daily |>
    left_join(cad_daily_forecast, by = "DOW")
}

# Fill any NAs and finalize
week_ahead_daily <- week_ahead_daily |>
  mutate(across(where(is.numeric), ~ replace_na(., 0))) |>
  arrange(Date)

# ============================================================================
# HOURLY FORECASTS - TOTAL EVENTS PER HOUR
# Using 5 weeks (840 hours) of historical data to forecast next 168 hours
# ============================================================================
week_ahead_hourly <- NULL
if (!is.null(cad_source) || !is.null(phone_source)) {
  # Build hourly CAD event baselines from all available historical data (5 weeks = 840 hours)
  hourly_cad <- NULL
  if (!is.null(cad_source)) {
    # Use same approach as hour-dow-analysis chunk
    cad_ts <- cad_source |>
      filter(Agency %in% c("POLICE","FIRE","EMS"))
    
    # Check if DOW and Hour already exist, if not create them
    if (!"DOW" %in% names(cad_ts) || !"Hour" %in% names(cad_ts)) {
      cad_ts <- cad_ts |>
        mutate(
          TS = if (inherits(Incident_Start_Time, "POSIXct")) {
            Incident_Start_Time
          } else {
            suppressWarnings(lubridate::mdy_hms(Incident_Start_Time))
          },
          DOW = factor(toupper(format(TS, "%a")), levels = c("SUN","MON","TUE","WED","THU","FRI","SAT"), ordered = TRUE),
          Hour = sprintf("%02d", lubridate::hour(TS))
        )
    } else {
      # Ensure Hour is zero-padded even if it already exists
      cad_ts <- cad_ts |>
        mutate(Hour = sprintf("%02d", as.numeric(as.character(Hour))))
    }
    ...
    # Count events by DOW and Hour (same as hour-dow-analysis)
    hourly_counts <- cad_ts |>
      group_by(DOW, Hour) |>
      summarise(call_count = n(), .groups = 'drop')
    
    # Calculate average per DOW+Hour combination
    # Get number of occurrences of each DOW to calculate average
    date_col <- NULL
    if ("Response_Date" %in% names(cad_ts)) {
      date_col <- "Response_Date"
    } else if ("Incident_Start_Time" %in% names(cad_ts)) {
    ...
    if (!is.null(date_col)) {
      ...
      hourly_cad <- hourly_counts |>
        left_join(date_counts, by = "DOW") |>
        mutate(TotalEvents = round(call_count / pmax(1, n_dates), 0)) |>
        dplyr::select(DOW, Hour, TotalEvents)
    } else {
      # Fallback: assume ~5 occurrences
      hourly_cad <- hourly_counts |>
        mutate(TotalEvents = round(call_count / 5, 0)) |>
        dplyr::select(DOW, Hour, TotalEvents)
    }
  }
  
  # Create hourly forecast grid
  future_hours <- expand_grid(
    Date = seq(future_start, future_end, by = "1 day"),
    Hour = sprintf("%02d", 0:23)
  ) |>
    mutate(
      DOW = factor(toupper(format(Date, "%a")), levels = c("SUN","MON","TUE","WED","THU","FRI","SAT"), ordered = TRUE),
      DateTime = as.POSIXct(paste0(Date, " ", Hour, ":00:00"))
    )
  
  week_ahead_hourly <- future_hours
  
  # Add phone forecasts (911 and Admin)
  if (!is.null(phone_baseline)) {
    week_ahead_hourly <- week_ahead_hourly |>
      left_join(phone_baseline, by = c("DOW", "Hour"))
  }
  
  # Add CAD total events per hour
  if (!is.null(hourly_cad)) {
    week_ahead_hourly <- week_ahead_hourly |>
      left_join(hourly_cad, by = c("DOW", "Hour"))
  }
  
  # Fill NAs and order columns
  week_ahead_hourly <- week_ahead_hourly |>
    mutate(across(c(Calls_911, Calls_Admin, TotalEvents), ~ replace_na(., 0))) |>
    dplyr::select(DateTime, Date, DOW, Hour, Calls_911, Calls_Admin, TotalEvents) |>
    arrange(DateTime)
}

# Persist daily forecast to CSV for downstream use
dir.create("report_files", showWarnings = FALSE, recursive = TRUE)
readr::write_csv(week_ahead_daily, file.path("report_files","week_ahead_daily.csv"))
if (!is.null(week_ahead_hourly)) {
  readr::write_csv(week_ahead_hourly, file.path("report_files","week_ahead_hourly.csv"))
}

# Display in report
cat("\n\n### Week Ahead Forecast (Daily)\n\n")
to_ft(week_ahead_daily, caption = paste0("Week Ahead (", format(future_start, "%d %b"), "–", format(future_end, "%d %b %Y"), ") – Daily Forecasts"))

# Display full hourly forecast for the week
if (!is.null(week_ahead_hourly)) {
  cat("\n\n### Week Ahead Forecast (Hourly - Full Week)\n\n")
  # Create formatted display table for hourly data
  hourly_display <- week_ahead_hourly |>
    dplyr::select(DateTime, Hour, DOW, Calls_911, Calls_Admin, TotalEvents) |>
    mutate(DateTime = as.character(DateTime))
  
  to_ft(hourly_display, 
        caption = paste0("Full Week Hourly Forecast (", format(future_start, "%A, %d %B %Y"), " – ", format(future_end, "%A, %d %B %Y"), ")"),
        header_map = list(
          DateTime = "Date/Time",
          Hour = "Hour",
          DOW = "Day",
