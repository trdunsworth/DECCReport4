...
## Week 51 from 14 December through 20 December 2025
...
build_phone_weekly_summary <- function(phone_df, week_num, week_start) {
  ...
  # Use T_AB for abandoned totals if available
  if ("T_AB" %in% names(phone_df)) {
    abandoned_total <- sum(phone_df$T_AB, na.rm = TRUE)
  } else {
    # Legacy calculation from individual AB columns
    abandoned_911 <- sum(phone_df$`911_AB`, na.rm = TRUE)
    abandoned_admin <- sum(phone_df$`ADM_AB`, na.rm = TRUE)
    abandoned_total <- abandoned_911 + abandoned_admin
  }
  
  # Calculate total calls and answered calls
  if ("T_RECD" %in% names(phone_df)) {
    total_calls <- sum(phone_df$T_RECD, na.rm = TRUE)
    answered_calls <- total_calls - abandoned_total
  } else {
    total_911 <- sum(phone_df$`911_T`, na.rm = TRUE)
    answered_calls <- total_911 - sum(phone_df$`911_AB`, na.rm = TRUE)
    total_calls <- total_911
  }
  ...
  tibble(
    ...
    abandoned_pct = (abandoned_total / total_calls) * 100,
    within_10 = within_10,
    within_15 = within_15,
    within_20 = within_20,
