...
if (!is.null(phone_trends_df) && nrow(phone_trends_df) > 0) {
  # Ensure proper ordering by week_start
  phone_trends_plot <- phone_trends_df %>% arrange(week_start)
  
  p_abandoned_count <- ggplot(phone_trends_plot, aes(x = reorder(week_label, week_start), y = abandoned_total)) +
    geom_col(fill = "#8B0000", width = 0.6, alpha = 0.85) +
    geom_text(aes(label = scales::comma(abandoned_total)),
              vjust = -0.3, size = 3.5, fontface = "bold", color = "#8B0000") +
