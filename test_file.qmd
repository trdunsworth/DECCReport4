...
if (!is.null(current_week_phone)) {
  ...
  # Calculate key metrics using T_RECD and T_AB if available, otherwise fall back to old columns
  if ("T_RECD" %in% names(current_week_phone)) {
    total_phone_calls <- sum(current_week_phone$T_RECD, na.rm = TRUE)
    total_abandoned <- sum(current_week_phone$T_AB, na.rm = TRUE)
    total_answered <- total_phone_calls - total_abandoned
  } else {
    total_phone_calls <- sum(current_week_phone$Total, na.rm = TRUE)
    total_abandoned <- sum(current_week_phone$`911_AB`, na.rm = TRUE) + sum(current_week_phone$ADM_AB, na.rm = TRUE)
    total_answered <- total_phone_calls - total_abandoned
  }
